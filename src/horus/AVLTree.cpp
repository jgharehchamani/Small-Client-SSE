...
string AVLTree::incrementSrcCnt(Node* head, Bid key) {
    ...
    if (head->key > key) {
        ...
        int src_cnt = stoi(res);
        string newval = to_string(src_cnt + 1);
...
string AVLTree::incrementUpdtCnt(Node* head, Bid key) {
    if (head == NULL || head->key == 0)
        return "";
    head = oram->ReadNode(head->key, head->pos, head->pos);
    if (head->key > key) {
        return incrementUpdtCnt(oram->ReadNode(head->leftID, head->leftPos, head->leftPos), key);
    } else if (head->key < key) {
        return incrementUpdtCnt(oram->ReadNode(head->rightID, head->rightPos, head->rightPos), key);
    } else {
        string res(head->value.begin(), head->value.end());
        int updt_cnt = stoi(res);
        string newval = to_string(updt_cnt + 1);
        std::fill(head->value.begin(), head->value.end(), 0);
        std::copy(newval.begin(), newval.end(), head->value.begin());
        oram->WriteNode(key, head);
        return res;
...
string AVLTree::decrementUpdtCnt(Node* head, Bid key) {
    if (head == NULL || head->key == 0)
        return "";
    head = oram->ReadNode(head->key, head->pos, head->pos);
    if (head->key > key) {
        return decrementUpdtCnt(oram->ReadNode(head->leftID, head->leftPos, head->leftPos), key);
    } else if (head->key < key) {
        return decrementUpdtCnt(oram->ReadNode(head->rightID, head->rightPos, head->rightPos), key);
    } else {
        string res(head->value.begin(), head->value.end());
        int updt_cnt = stoi(res);
        string newval = to_string(updt_cnt - 1);
