...
string OMAP::incrementUpdtCnt(Bid key) {
    string res = "";
    treeHandler->startOperation();
    Node* node = new Node();
    node->key = rootKey;
    node->pos = rootPos;
    res = treeHandler->incrementUpdtCnt(node, key);
    treeHandler->finishOperation(true, rootKey, rootPos);
    return res;
}

string OMAP::decrementUpdtCnt(Bid key) {
    string res = "";
    treeHandler->startOperation();
    Node* node = new Node();
    node->key = rootKey;
    node->pos = rootPos;
    res = treeHandler->decrementUpdtCnt(node, key);
    treeHandler->finishOperation(true, rootKey, rootPos);
    return res;
}